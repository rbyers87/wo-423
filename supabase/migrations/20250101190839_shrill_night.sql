/*
            ...
            */
            ...
            -- Add likes and dislikes count to messages
            ALTER TABLE messages
            ADD COLUMN likes int GENERATED ALWAYS AS (
              SELECT count(*)
              FROM likes
              WHERE likes.profile_id = messages.profile_id
            ) STORED;
    
            ALTER TABLE messages
            ADD COLUMN dislikes int GENERATED ALWAYS AS (
              SELECT count(*)
              FROM dislikes
              WHERE dislikes.profile_id = messages.profile_id
            ) STORED;
